...
export function Sidebar({ user }: Props) {
    const admin = (window as any).ticketflowAdmin || {};
    const companyLogo = admin.companyLogo || '';
    const companyName = admin.companyName || 'Support';
        ...
        <aside className="tf-w-56 tf-bg-white tf-border-r tf-border-gray-200 tf-flex tf-flex-col">
            <div className="tf-p-4 tf-border-b tf-border-gray-200">
                {companyLogo === 'kleinbooks' ? (
                    ...
                    <img src={companyLogo} alt={companyName} className="tf-h-8 tf-w-auto" />
                ) : (
                    <h1 className="tf-text-lg tf-font-bold tf-text-primary-600">{companyName}</h1>
                )}
            </div>
