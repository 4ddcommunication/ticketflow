    ...
    return (
        <div>
            <div className="tf-flex tf-justify-between tf-items-center tf-mb-6">
                <h2 className="tf-text-2xl tf-font-bold tf-text-gray-900">Saved Replies</h2>
                <button
            ...
            <div className="tf-bg-white tf-rounded-lg tf-shadow-sm tf-border tf-border-gray-200">
                {loading ? (
                    ...
                    <p className="tf-py-12 tf-text-center tf-text-gray-500">No saved replies yet.</p>
                ) : (
                        ...
                        {replies.map((reply) => (
                            <div key={reply.id} className="tf-px-4 tf-py-3">
                                <div className="tf-flex tf-justify-between tf-items-start">
                                    ...
                                    <div className="tf-flex tf-gap-2">
                                        <button onClick={() => handleEdit(reply)} className="tf-text-xs tf-text-primary-600 hover:tf-text-primary-700">Edit</button>
                                        <button onClick={() => handleDelete(reply.id)} className="tf-text-xs tf-text-red-600 hover:tf-text-red-700">Delete</button>
                                    </div>
                                </div>
                                <p className="tf-text-sm tf-text-gray-600 tf-mt-1 tf-line-clamp-2">{reply.body}</p>
                                <p className="tf-text-xs tf-text-gray-400 tf-mt-1">Used {reply.use_count} times</p>
                            </div>
                        ))}
                    </div>
                )}
